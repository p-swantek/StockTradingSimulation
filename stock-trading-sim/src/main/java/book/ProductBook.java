 ...
 * 
 * 
 * A class that represents the booked tradables for both the BUY and the SELL
 * side. This class will maintain a reference to two ProductBookSide objects,
 * one representing the BUY side and the other representing the SELL side. This
 * class will maintain a record of the stock product that it is associated with,
 * the latest market data of the stock, a list of Quotes for each user, and a
 * record of tradables that have been completely traded or cancelled.
 * 
...
public final class ProductBook {
    ...
    private Set<String> userQuotes;
    private Map<Price, ArrayList<Tradable>> oldEntries; // Old tradables that have been traded/cancelled

    /**
     * Constructs a new ProductBook for the associated stock symbol
     * 
     * @param newStockSymbol The stock symbol of the stock associated with this
     *            ProductBook
     * @throws InvalidDataException If the stock symbol passed in is null or
     *             empty
     */
    public ProductBook(String newStockSymbol) throws InvalidDataException {
        ...
        latestMarketData = "";
        userQuotes = new HashSet<>(); // Current Quotes for users
    ...
    /**
     * Gets all the Orders from the BUY and SELL ProductBookSides that have any
     * remaining volume
     * 
     * @param userName The user for which order information is desired
     * @return A list of buy and sell order information for orders that have
     *         remaining quantity
    ...
    public synchronized List<TradableDTO> getOrdersWithRemainingQty(String userName) throws InvalidDataException {
        ...
        List<TradableDTO> tempList = new ArrayList<>();
        tempList.addAll(buySide.getOrdersWithRemainingQty(name));
        tempList.addAll(sellSide.getOrdersWithRemainingQty(name));

        return tempList;
    }

    /**
     * Checks to see if an Order that was cancelled is actually able to be
     * cancelled. Will be too late to cancel the Order if it is already
     * contained in the old entries record
     * 
    ...
    public synchronized void checkTooLateToCancel(String orderId) throws InvalidDataException, OrderNotFoundException {
        ...
        for (Entry<Price, ArrayList<Tradable>> entry : oldEntries.entrySet()) {
            for (Tradable order : entry.getValue()) {
                if (order.getId().equals(orderId)) {
                    ...
                    CancelMessage cm = new CancelMessage(order.getUser(), order.getProduct(), order.getPrice(), order.getRemainingVolume(), "Too late to cancel.", order.getSide(), order.getId());
        ...
        throw new OrderNotFoundException("Error: The ProductBook couldn't locate the Order [" + orderId + "] to see if it was too late to cancel.");
     ...
     * 
     * @return 2D String array representing the book depth of the buy and sell
     *         sides of the product book
     */
    ...
    public synchronized void openMarket() throws InvalidDataException {
        ...
        while (buyPrice.greaterOrEqual(sellPrice) || buyPrice.isMarket() || sellPrice.isMarket()) {
            List<Tradable> topOfBuySide = buySide.getEntriesAtPrice(buyPrice);
            Map<String, FillMessage> allFills = null;
    ...
    public synchronized void cancelOrder(String side, String orderId) throws InvalidDataException, OrderNotFoundException {
        ...
        else if (side.trim().toUpperCase().equals("BUY")) {
    ...
    public synchronized void updateCurrentMarket() {
        ...
        String data = "" + buySideTopOfBookPrice + buySide.topOfBookVolume() + sellSideTopOfBookPrice + sellSide.topOfBookVolume();

        if (!latestMarketData.equals(data)) {
            MarketDataDTO info = new MarketDataDTO(getStockSymbol(), buySideTopOfBookPrice, buySide.topOfBookVolume(), sellSideTopOfBookPrice, sellSide.topOfBookVolume());
    ...
    private synchronized Price determineLastSalePrice(Map<String, FillMessage> fills) throws InvalidDataException {
        if (fills == null) {
    ...
    private synchronized int determineLastSaleQuantity(Map<String, FillMessage> fills) throws InvalidDataException {
        if (fills == null) {
    ...
    private synchronized void addToBook(String side, Tradable trd) throws InvalidDataException {
        ...
        else if (ProductService.getInstance().getMarketState().equals("PREOPEN")) {
