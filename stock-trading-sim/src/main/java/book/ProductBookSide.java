...
/**
 * This class represents either the BUY or the SELL side of a ProductBook for a
 * certain stock. The ProductBookSide will maintain a list of tradables that are
 * trading at a certain Price. The Buy Side will be represented by a descending
 * list of Prices while the Sell side will be represented by an ascending list
 * of Prices. The class will also keep a record of the side it represents, a
 * reference to the ProductBook to which it belongs, as well as a reference to
 * an implementor of the TradeProcessor interface (this will actually perform
 * the trades).
 * 
...
public class ProductBookSide {
    ...
    private String side;
    private Map<Price, ArrayList<Tradable>> bookEntries; // Record of Prices and tradables
    ...
    private void setSide(String newSide) throws InvalidDataException {
        ...
        else if (!newSide.trim().toUpperCase().equals("BUY") && !newSide.trim().toUpperCase().equals("SELL")) {
    ...
    public synchronized List<TradableDTO> getOrdersWithRemainingQty(String userName) throws InvalidDataException {
        if (userName == null || userName.trim().isEmpty()) {
            throw new InvalidDataException("Error: ProductBookSide tried to obtain orders for a null or empty user name.");
        }

        ArrayList<TradableDTO> results = new ArrayList<>();

        HashMap<Price, ArrayList<Tradable>> tempMap = new HashMap<>(bookEntries);
        for (Entry<Price, ArrayList<Tradable>> entry : tempMap.entrySet()) {
            for (Tradable tradable : entry.getValue()) {
                if (userName.trim().toUpperCase().equals(tradable.getUser()) && tradable.getRemainingVolume() > 0) {
                    ...
                    TradableDTO orderInfo = new TradableDTO(tradable.getProduct(), tradable.getPrice(), tradable.getOriginalVolume(), tradable.getRemainingVolume(), tradable.getCancelledVolume(),
                            tradable.getUser(), tradable.getSide(), tradable.isQuote(), tradable.getId());
    ...
    synchronized List<Tradable> getEntriesAtTopOfBook() {
        if (bookEntries.isEmpty()) {
            return null;
        }

        ArrayList<Price> sortedList = new ArrayList<>(bookEntries.keySet());
    ...
    synchronized List<Tradable> getEntriesAtPrice(Price price) throws InvalidDataException {
        if (price == null) {
            throw new InvalidDataException("Error: ProductBookSide tried to get entries on a null Price.");
        }

        else if (!bookEntries.containsKey(price)) {
            return null;
        }
    ...
    public synchronized boolean hasMarketPrice() {
        ArrayList<Price> priceList = new ArrayList<>(bookEntries.keySet());
    ...
    public synchronized void cancelAll() throws OrderNotFoundException, InvalidDataException {
        ...
        ArrayList<Price> tempKeyList = new ArrayList<>(bookEntries.keySet());
        HashMap<Price, ArrayList<Tradable>> tempMap = new HashMap<>(bookEntries);

        for (Price p : tempKeyList) {
            ArrayList<Tradable> tempList = new ArrayList<>(tempMap.get(p));
        ...
        for (Price p : tempKeyList) {
            ...
            for (Tradable tradable : tempList) {
                if (tradable.getUser().equals(user.trim().toUpperCase())) {
                    ...
                    TradableDTO tradableInfo = new TradableDTO(tradable.getProduct(), tradable.getPrice(), tradable.getOriginalVolume(), tradable.getRemainingVolume(), tradable.getCancelledVolume(),
                            tradable.getUser(), tradable.getSide(), tradable.isQuote(), tradable.getId());
        ...
        for (Price p : tempKeyList) {
            ...
            for (Tradable tradable : tempList) {
                if (tradable.getId().equals(orderId)) {
                    ...
                    CancelMessage cm = new CancelMessage(tradable.getUser(), tradable.getProduct(), tradable.getPrice(), tradable.getRemainingVolume(), tradable.getSide() + " Order Cancelled",
    ...
    public synchronized void submitQuoteCancel(String userName) throws InvalidDataException {
        ...
        CancelMessage cm = new CancelMessage(results.user, results.product, results.price, results.remainingVolume, "Quote " + results.side + "-Side Cancelled", results.side, results.id);
    ...
    public synchronized void addToBook(Tradable trd) throws InvalidDataException {
        ...
        else if (!bookEntries.containsKey(trd.getPrice())) {
    ...
    public Map<String, FillMessage> tryTrade(Tradable trd) throws InvalidDataException {
        if (trd == null) {
            throw new InvalidDataException("Error: ProductBookSide attempted to trade a null Tradable.");
        }

        Map<String, FillMessage> allFills;
        if (getSide().equals("BUY")) {
    ...
    public synchronized Map<String, FillMessage> trySellAgainstBuySideTrade(Tradable trd) throws InvalidDataException {
        if (trd == null) {
            throw new InvalidDataException("Error: ProductBookSide attempted a SellAgainstBuySideTrade with a null Tradable.");
        }

        Map<String, FillMessage> allFills = new HashMap<>();
        Map<String, FillMessage> fillMsgs = new HashMap<>();

        while ((trd.getRemainingVolume() > 0 && !isEmpty()) && (trd.getPrice().isMarket() || trd.getPrice().lessOrEqual(topOfBookPrice()))) {

            Map<String, FillMessage> temp = executor.doTrade(trd);
    ...
    private Map<String, FillMessage> mergeFills(Map<String, FillMessage> existing, Map<String, FillMessage> newOnes) throws InvalidDataException {
    ...
    public synchronized Map<String, FillMessage> tryBuyAgainstSellSideTrade(Tradable trd) throws InvalidDataException {
        if (trd == null) {
