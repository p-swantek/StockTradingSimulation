 ...
 * 
 * A class that provides an implementation of the TradeProcessor interface.  This particular class will provide
 * an implementation of the PriceTime trading algorithm. The class maintains a record of FillMessages for trades as well
 * as a reference to the ProductBookSide object with which it is associated.
 *
 */
...
public class TradeProcessorPriceTimeImpl implements TradeProcessor {
	...
	public HashMap<String, FillMessage> doTrade(Tradable trd) throws InvalidDataException {
		...
		ArrayList<Tradable> entriesAtPrice = theOwner.getEntriesAtTopOfBook();
		...
		for (Tradable t : entriesAtPrice){
			...
			if (trd.getRemainingVolume() >= t.getRemainingVolume()){
				...
				FillMessage tFillMessage = new FillMessage(t.getUser(), t.getProduct(), tradePrice, t.getRemainingVolume(),
								"leaving 0", t.getSide(), t.getId());
				
				addFillMessage(tFillMessage);
				
				FillMessage trdFillMessage = new FillMessage(trd.getUser(), trd.getProduct(), tradePrice, t.getRemainingVolume(),
				...
				FillMessage tFillMessage = new FillMessage(t.getUser(), t.getProduct(), tradePrice, trd.getRemainingVolume(),
								"leaving " + remainder, t.getSide(), t.getId());
				
				addFillMessage(tFillMessage);
				
				FillMessage trdFillMessage = new FillMessage(trd.getUser(), trd.getProduct(), tradePrice, trd.getRemainingVolume(),
