...
/**
 * A Price Factory to create the different types of Price objects. Implements
 * the flyweight design pattern, keeps a record of previously created Price
 * objects in a HashMap. If requested to make a certain Price object, the
 * factory checks if that Price has been created already and returns a reference
 * to it if it has. Returns a new Price instance if one has not been created
 * yet. The factory also creates a single instance of a market price and keeps a
 * reference to it. When a request is made for a market price object, this
...
public class PriceFactory {
    ...
    private static final Map<String, Price> pricesCreated = new HashMap<>(); // The record of created Price objects.
    private static final Price marketPrice = new Price(); // Only keep a reference to a single market price reference.
