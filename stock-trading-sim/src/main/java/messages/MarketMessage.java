 ...
 * 
 * A class to represent the state of the market. A valid market state can only be the represented by the Strings: "OPEN", "PREOPEN", and "CLOSED"
 * the set method makes sure that the market state is neither null or an empty String, will only allow the state to be set to a valid state.
 *
 */
...
public class MarketMessage {
	...
	private void setState(String newState) throws InvalidDataException {
		...
		if (!newState.trim().toUpperCase().equals("CLOSED") && !newState.trim().toUpperCase().equals("PREOPEN") &&
				!newState.trim().toUpperCase().equals("OPEN"))
